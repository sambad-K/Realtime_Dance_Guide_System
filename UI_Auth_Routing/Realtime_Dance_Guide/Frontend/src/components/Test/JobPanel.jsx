...
export default function VideoJobPanel({ kind, label, onReady, initialFile = null }) {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (j.status === "done") {
          ...
          if (!alive) return;
          // ✅ Attach the video file to preview so Live component can access it
          onReady({ kind, jobId, preview: { ...preview, file } });
        }
      } catch {}
    }, 1500);
    ...
    <div className="job-panel">
      ...
      <div className="file-input-wrapper">
        ...
        <label htmlFor={`file-input-${kind}`} className="file-input-label">
          ...
          {file && (
            <span className="file-input-filename">{file.name}</span>
          )}
        </label>
      </div>
        ...
        <>
          ...
          {status !== "uploading" && status !== "processing" && (
            <div className={`status-badge ${status}`}>
              <span className="status-icon">
                {status === "idle" ? "⏳" : status === "queued" ? "⏱️" : status === "done" ? "✅" : "❌"}
              </span>
          ...
          {(status === "uploading" || status === "processing") && (
            (() => {
              ...
              const small = pct < 12;
              return (
                <div className="progress-bar-unique" data-percent={small ? "small" : "normal"} aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} role="progressbar">
                  <span className="progress-text-processing">{status === "uploading" ? "Uploading..." : "Processing..."}</span>
                  <div className="progress-bar-fill-unique" style={{ width: `${pct}%` }}>
                    <span className="progress-text-unique">{pct}%</span>
                  </div>
                </div>
              );
            })()
